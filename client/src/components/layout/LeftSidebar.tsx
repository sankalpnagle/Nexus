...
import { Home, Users, MessageCircle, Grid3x3, Bookmark, Clock, ChevronRight, Zap, MessageSquare } from 'lucide-react';
...
export default function LeftSidebar() {
  ...
  const onlineFriends = friends.filter(f => typeof f === 'object' && (online[f._id] || f.isOnline));
  ...
  return (
    <aside className="w-64 shrink-0 sticky top-14 h-[calc(100vh-56px)] overflow-y-auto py-4 px-2 hidden lg:flex flex-col gap-1">
      ...
      >
        ...
        <div>
          <p className="text-sm font-semibold transition-colors" style={{ color: 'var(--nx-text)' }}>{user?.firstName} {user?.lastName}</p>
      ...
      <div>
        <div className="flex items-center gap-2 px-3 mb-2">
          ...
          <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: M }}>
            Online Friends
        ...
        {onlineFriends.length === 0 ? (
          <p className="px-3 text-xs" style={{ color: M }}>No friends online right now</p>
        ) : (
          onlineFriends.slice(0, 10).map(f => (
            ...
            >
              ...
              <button onClick={() => nav(`/profile/${f._id}`)} className="shrink-0">
              ...
              <button
                ...
                onMouseLeave={e => { (e.currentTarget.style.background = 'rgba(124,111,247,0.15)'); (e.currentTarget.style.color = A); }}
