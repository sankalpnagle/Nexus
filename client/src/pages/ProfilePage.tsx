...
export default function ProfilePage() {
  ...
  const isFriend = profile?.friends.some(
    (f) => (typeof f === "string" ? f : (f as User)._id) === me?._id,
  );
  ...
  const loadPosts = async () => {
    try {
      const r = await api.get(`/posts/user/${userId}`);
  ...
  const respondReq = async (action: "accept" | "decline") => {
    try {
      await api.put(`/users/${userId}/friend-request`, { action });
      doRefresh();
    } catch {
      toast.error("Failed");
    }
  ...
  const unfriend = async () => {
    try {
      await api.delete(`/users/${userId}/unfriend`);
      doRefresh();
  ...
  if (!profile)
    return (
      <div className="text-center py-16 text-[var(--nx-muted)]">
        User not found
      </div>
    );
  ...
  const aboutItems = [
    ...
    },
    {
      icon: Calendar,
      label: "Joined",
      val: format(new Date(profile.createdAt), "MMMM yyyy"),
    },
  ];
  ...
  return (
    <div className="max-w-[980px] mx-auto pb-8">
        ...
        {isOwner && (
          <label className="absolute bottom-4 right-4 flex items-center gap-1.5 bg-[var(--nx-surface)]/90 backdrop-blur-sm text-[var(--nx-text)] text-xs font-semibold px-3 py-2 rounded-xl cursor-pointer hover:bg-[var(--nx-card)] transition-colors border border-[var(--nx-border-2)]">
            ...
            <input
              ...
              onChange={(e) =>
                e.target.files?.[0] &&
                changePhoto("coverPhoto", e.target.files[0])
              }
            />
          </label>
        )}
      ...
      <div className="bg-[var(--nx-surface)] border border-[var(--nx-border)] border-t-0 px-4 sm:px-6 pb-4">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          <div className="flex items-end gap-4 -mt-14 sm:-mt-16">
              ...
              {isOwner && (
                <label className="absolute bottom-2 right-2 w-8 h-8 bg-[var(--nx-surface)] border border-[var(--nx-border-2)] rounded-xl flex items-center justify-center cursor-pointer hover:bg-[var(--nx-border)] transition-colors">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files?.[0] &&
                      changePhoto("avatar", e.target.files[0])
                    }
                  />
                </label>
              )}
            ...
            <div className="pb-2">
              ...
              <p className="text-[var(--nx-muted)] text-sm mt-0.5">
              ...
              <div className="flex mt-1.5">
                {friends.slice(0, 5).map((f, i) => (
                  <img
                    ...
                    alt=""
                    className={cx(
                      "w-6 h-6 rounded-full border-2 border-[var(--nx-surface)] object-cover",
                      i > 0 && "-ml-1.5",
                    )}
                  />
                ))}
            ...
            {isOwner ? (
              ...
              <>
                <Button
                  ...
                  onClick={() => respondReq("accept")}
                >
                  Confirm
                </Button>
                <Button
                  variant="secondary"
                  onClick={() => respondReq("decline")}
                >
                  Delete
                </Button>
              </>
        ...
        {/* Bio preview */}
        {profile.bio && !editing && (
          <p className="text-[var(--nx-subtle)] text-sm mt-3 border-t border-[var(--nx-border)] pt-3">
            {profile.bio}
          </p>
        )}
        ...
        <Card className="p-5 mt-4 mx-2 sm:mx-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
            ...
            <Input
              ...
              placeholder="https://…"
              onChange={(e) =>
                setEditData((d) => ({ ...d, website: e.target.value }))
              }
            />
            <Input
              label="Occupation"
              ...
              onChange={(e) =>
                setEditData((d) => ({ ...d, occupation: e.target.value }))
              }
            />
            <Input
              label="Education"
              ...
              onChange={(e) =>
                setEditData((d) => ({ ...d, education: e.target.value }))
              }
            />
          </div>
      ...
      <div className="mt-4 px-0 sm:px-0">
        ...
        {tab === "posts" && (
          <div className="grid grid-cols-1 lg:grid-cols-[300px,1fr] gap-4 px-2 sm:px-0">
            ...
            <div className="space-y-3">
              <Card className="p-4">
                <h3 className="font-bold text-[var(--nx-heading)] mb-3 font-[var(--font-display)]">
                  Intro
                </h3>
                      ...
                      >
                        ...
                        {link ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-[#7c6ff7] hover:underline truncate"
                          >
                            {val}
                          </a>
                        ) : (
                          <strong className="text-[var(--nx-text)]">
                            {val}
                          </strong>
                        )}
                      </div>
              ...
              {photos.length > 0 && (
                <Card className="p-4">
                  <div className="flex justify-between items-center mb-3">
                    <h3 className="font-bold text-[var(--nx-heading)] font-[var(--font-display)]">
                      Photos
                    </h3>
                    <button
                      onClick={() => setTab("photos")}
                      className="text-xs text-[#7c6ff7] hover:text-[#6459e0]"
                    >
                      See all
                    </button>
                  </div>
                  <div className="grid grid-cols-3 gap-0.5 rounded-xl overflow-hidden">
                    {photos.slice(0, 9).map((url, i) => (
                      <img
                        ...
                        src={url}
                        alt=""
                        className="aspect-square object-cover hover:opacity-80 cursor-pointer transition-opacity"
                      />
                    ))}
                  </div>
                </Card>
        ...
        {tab === "about" && (
          <Card className="p-6 mx-2 sm:mx-0">
            <h3 className="font-bold text-[var(--nx-heading)] text-lg mb-5 font-[var(--font-display)]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {aboutItems
                ...
                .map(({ icon: Icon, label, val, link }) => (
                  <div
                    key={label}
                    className="flex items-center gap-3 p-3.5 bg-[var(--nx-surface)] border border-[var(--nx-border)] rounded-xl"
                  >
                    ...
                    <div>
                      <p className="text-[10px] text-[var(--nx-muted)] font-bold uppercase tracking-wider">
                        {label}
                      </p>
                      {link ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-[#7c6ff7] hover:underline text-sm"
                        >
                          {val}
                        </a>
                      ) : (
                        <p className="text-[var(--nx-text)] font-semibold text-sm">
                          {val}
                        </p>
                      )}
                    </div>
